...
// MARK: - Coordinator
protocol Coordinator: AlertScene, LoadScene {
...
extension Coordinator {
    func showErrorAlert(_ error: String, handler: ((UIAlertAction) -> Void)? = nil) {
        DispatchQueue.main.async {
            guard let view = self.navigationController.topViewController else {
                return
            }
            view.alert(message: error, handler: nil)
        }
    }
}
...
extension Coordinator {
    func startLoading(_ lockType: LoadingLockType) {
        DispatchQueue.main.async {
            ...
            view.startLoading(lockType: lockType)
        }
    }
    ...
    func stopLoading(_ lockType: LoadingLockType) {
        DispatchQueue.main.async {
            ...
            view.stopLoading(lockType: lockType)
        }
    }
}
